...
class activity_navigation implements renderable, templatable {
    ...
    public function __construct($prevmod, $nextmod, $activitylist = array()) {
        ...
        if ($prevmod) {
            ...
            $this->prevlink = new \action_link($linkurl, '<span id="activity_nav_prev_arrow">'.$OUTPUT->larrow().'</span>' . ' ' .'<span id="activity_nav_prev_text">'.$linkname.'</span>', null, $attributes);
        }
        ...
        if ($nextmod) {
            ...
            $this->nextlink = new \action_link($linkurl, '<span id="activity_nav_next_text">'.$linkname.'</span>' . ' ' .'<span id="activity_nav_next_arrow">'. $OUTPUT->rarrow().'</span>', null, $attributes);
        }
