...
if (!empty($data) || (!empty($p) && !empty($s))) {
    ...
    if ($confirmed == AUTH_CONFIRM_ALREADY) {
        ...
        if (!$user = get_complete_user_data('username', $username, $id)) {
        ...
        echo $OUTPUT->header();
        echo $OUTPUT->box_start('generalbox centerpara boxwidthnormal boxaligncenter');
        echo "<h3>"."ကြိုဆိုပါတယ်, ". fullname($USER) . "</h3>\n";
        echo "<p>"."ကျောင်းသားမှတ်ပုံတင်ခြင်းကို အောင်မြင်စွာ အတည်ပြုပြီးပါပြီ
။"."</p>\n";
        echo $OUTPUT->single_button(core_login_get_return_url(), 'နှိပ်ပါ');
        echo $OUTPUT->box_end();
        echo $OUTPUT->footer();
        exit;
    } else {
        print_error($confirmed);
    }
} else {
